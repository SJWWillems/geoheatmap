...
# TO DO (remove this list later): 

- remove graph names "one", "two" etc. Is that possible? Just add "+" at end of the first line of code?
- change structure of code -> align arguments, etc.
- remove interpretations of the plot as they are not necessarily relevant for how to make the plot? Or was this also done in other vignettes? 


The geoheatmap R package aims to provide an easy way for building cartogram heatmaps for regions worldwide. 

For the graph aesthetics, we took inspirations from the maps of the [statebins](https://github.com/hrbrmstr/statebins) package, and extended on its functionality by allowing for user-defined grids, which means that the cartogram heatmaps can also represent territories outside the US. Though any well-defined grid will technically work, grids from  [geofacet](https://github.com/hafen/geofacet) are an excellent starting point.


## Installation and Setup

The package `geoheatmap` imports grids from the [geofacet](https://github.com/hafen/geofacet) package, and also includes the option to make hoovering graphs, which is based on functionalities from the [plotly](https://github.com/plotly/plotly.R) package.
...
In this vignette we show multiple implementations of the package, namely for continuous data and discrete data, and we will show how users can specify the column with the grid names and can make an interactive (hoovering) plot instead of a stationary one. 
